...
from app.agent.fc_client import call_with_tools, call_simple, MODEL_NAME
from app.agent.clarifier import extract_scope
from app.agent.loop.helpers import QUOTA_FALLBACK, _ev_tag, _grade_topic_from_analysis, _load_memory_block
...
from app.agent.loop.pipeline import deduplicate_hits, critic_filter, rerank_top
...
async def run_agentic_rag_stream(client, kb, question, history=None, max_iterations=4, analysis=None, teacher_id="default", light=False):
    ...
    all_hits = rerank_top(client, question, all_hits, top_k=10)
    if len(all_hits) > 6:
        yield {"type": "status", "message": "يعيد ترتيب النتائج..."}

    try:
        if _is_pedagogy_query(question, analysis):
            _ped_stream = _fetch_pedagogy_hits(kb, client, question, question_scope)
            if _ped_stream:
                all_hits = _merge_pedagogy(all_hits, _ped_stream, ped_first=True, limit=14)
                trace.append({"tool": "searchChunks:pedagogy_tier2", "hits": len(_ped_stream)})
    except Exception as e:
        print(f"[Pedagogy stream merge skip: {e}]")
