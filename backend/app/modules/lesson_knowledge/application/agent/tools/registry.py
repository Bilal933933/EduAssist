...
def _load_manifests():
    ...
    for entry in os.listdir(_TOOLS_DIR):
        ...
        mod = importlib.import_module(f"app.modules.lesson_knowledge.application.agent.tools.{entry}.tool")
...
try:
    from app.modules.lesson_knowledge.application.agent.tools.readFile.tool import _resolve_safe_path, _smart_read, ALLOWED_ROOTS
except Exception:
    ALLOWED_ROOTS = []
