...
from src.agent.tools.registry import TOOL_DECLARATIONS, execute_tool, ALLOWED_ROOTS

try:
    from src.agent.tools.searchChunks.tool import format_search_observation
except Exception:
    def format_search_observation(hits, max_hits=8, max_chars=600):
        return "لا توجد نتائج بحث."

try:
    from src.agent.tools.readFile.tool import _smart_read, _resolve_safe_path
except Exception:
