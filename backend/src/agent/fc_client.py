...
def _try_generate(client, model, contents, config):
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            print(f"(429 — إعادة خلال {wait:.0f}ث)")
            time.sleep(wait)
    raise last
...
def call_simple(client, user: str, system: str) -> str:
    ...
    for i, model in enumerate(models):
        try:
            ...
            for attempt in range(MAX_RETRIES):
                try:
                    r = client.models.generate_content(model=model, contents=user, config=types.GenerateContentConfig(system_instruction=system))
                    return r.text
                except Exception as e:
                    last = e
                    if not _is_retryable(e) or attempt == MAX_RETRIES-1:
